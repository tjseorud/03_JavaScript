...
btn.addEventListener('click',function(){
  ...
  for(let i=0;i<fontJc.length;i++){ /* 가로정렬 */
    fontJc[i].addEventListener('change',function(e){display.style.justifyContent =e.target.value});
  }
  for(let i=0;i<fontAi.length;i++){ /* 세로정렬 */
    fontAi[i].addEventListener('change',function(e){display.style.alignItems =e.target.value});
  }
  display.innerHTML =inText.value;

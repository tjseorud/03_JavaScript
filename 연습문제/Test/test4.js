...
const userBtn = document.querySelectorAll(".user-btn > button");  /* 가위 바위 보 버튼들 */
const startBtn = document.querySelector("#startBtn"); /* 시작 버튼 */
const score = document.querySelectorAll("#score > th"); /* 점수판 */
const gameHistory = document.querySelector("#gameHistory"); /* 게임 히스토리 */ 
let gameCount = 1;  /* 현재 게임 판 수 */
let maxCount; /* 최대 게임 판 수 */
...
for(let i = 0 ; i < userBtn.length; i++){
  userBtn[i].addEventListener("click", () => {
    ...
    }else if((userChoice === '가위' && comChoice === '보') || (userChoice === '바위' && comChoice === '가위') || (userChoice === '보' && comChoice === '바위')){ // user가 이긴 경우

...
const displays =document.querySelectorAll('.box');  /* radio */
const radios =document.querySelectorAll('.opacity-container > input');
// const radios =document.querySelectorAll('[name=opacity-btn]');

/* index -> 요소 탐색 코드 */
// 향상된 for : 0-끝 까지 모든 요소를 순차 접근하는 용도
for(let input of colors){ /* input ===colors[i], i는 0-끝 까지 +1 */
  input.addEventListener('keyup',()=>{
    input.previousElementSibling.style.backgroundColor =input.value; /* 입력된 input의 이전 형제(div.box)의 배경색 변경 */
  });
}
